...
export function HeroSection() {
  ...
  return (
    <section ref={ref} className="relative overflow-hidden border-b border-line">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_72%_38%,rgba(124,156,255,0.14),transparent_34%)]" aria-hidden="true" />
      <div className="relative mx-auto grid min-h-[calc(100vh-4rem)] max-w-7xl items-center gap-12 px-6 py-20 sm:px-8 lg:grid-cols-[1fr_0.9fr] lg:gap-16 lg:px-10 lg:py-24">
        <motion.div style={{ y, opacity }} className="max-w-2xl">
          ...
          >
            See how it fits before it arrives.
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Explore products in 3D, place them in your space, and buy with the confidence that comes from seeing the details first.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <span className="flex items-center gap-2"><Eye className="h-4 w-4 text-electric" /> 3D and AR-ready catalog</span>
            <span className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-accent" /> No account needed to browse</span>
          </motion.div>
        </motion.div>
        ...
        >
          <div className="mb-3 flex items-center justify-between px-1 text-xs text-muted">
            <span className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-accent" /> Live product stage</span>
            <span className="flex items-center gap-1"><Sparkles className="h-3.5 w-3.5" /> Rotate to inspect</span>
          </div>
